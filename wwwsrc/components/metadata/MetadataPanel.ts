...
export interface MetadataPanelProps {
    ...
    metadata: Res.MetadataItemResponse[];
...
export function MetadataPanel(props: MetadataPanelProps): VNode {
    ...
    const groups: { [k: string]: Res.MetadataItemResponse[] } = {};
    metadata.forEach(m => { 
    ...
    if (!groups['File Info']) groups['File Info'] = [];
    ...
    return h('div.metadata-panel', {
        ...
        (hasAperture && hasFocal) ? ApertureVisualizer({ metadata: metadata as any, cameraThumbUrl }) : null,
