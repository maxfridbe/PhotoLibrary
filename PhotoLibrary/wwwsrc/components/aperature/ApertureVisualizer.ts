...
export function ApertureVisualizer(data: ApertureData): VNode {
    const { metadata, cameraThumbUrl } = data;

    const getVal = (dir: string, tag: string): string | number | null => {
        const item = metadata.find((x) => x.directory === dir && x.tag === tag);
        return item ? item.value : null;
    };

    const parseNum = (input: string | number | null): number => {
    ...
    const explicitBlades =
        parseNum(getVal("MakerNotes", "Aperture Blades")) ||
        parseNum(getVal("Composite", "Aperture Blades"));
    const bladeCountVal: number = explicitBlades > 0 ? explicitBlades : 7;
    ...
    if (cropFactor >= 6.0) { sensorName = "1/2.7\" Small Sensor"; sensorW_mm = 5.37; sensorH_mm = 4.04; }
    else if (cropFactor >= 5.0) { sensorName = "1/2.3\" Small Sensor"; sensorW_mm = 6.17; sensorH_mm = 4.55; }
    else if (cropFactor >= 4.0) { sensorName = "1/1.7\" Point & Shoot"; sensorW_mm = 7.6; sensorH_mm = 5.7; }
    else if (cropFactor >= 2.6) { sensorName = '1" / CX'; sensorW_mm = 13.2; sensorH_mm = 8.8; }
    else if (cropFactor >= 1.9) { sensorName = "Micro 4/3"; sensorW_mm = 17.3; sensorH_mm = 13; }
    else if (cropFactor >= 1.6) { sensorName = "APS-C (Canon)"; sensorW_mm = 22.3; sensorH_mm = 14.9; }
    else if (cropFactor >= 1.3) { sensorName = "APS-C / DX"; sensorW_mm = 23.6; sensorH_mm = 15.6; }
    else if (cropFactor <= 1.1) { sensorName = "Full Frame"; sensorW_mm = 36; sensorH_mm = 24; }
