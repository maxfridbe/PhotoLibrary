...
export class LibraryManager {
    ...
    initLayout(containerId, triggerScanCallback) {
        ...
        const self = this;
        this.libraryLayout.registerComponent('stats', function (container) {
            ...
            setTimeout(() => { if (self.infoCache)
                self.renderStats(self.infoCache); }, 50);
        });
        this.libraryLayout.registerComponent('scan', function (container) {
            const el = document.createElement('div');
            el.className = 'gl-component lib-pane';
            el.style.padding = '1em';
        ...
        this.libraryLayout.registerComponent('foundFiles', function (container) {
            const el = document.createElement('div');
            el.className = 'gl-component lib-pane';
            el.style.padding = '1em';
        ...
        setTimeout(() => { if (this.libraryLayout)
    ...
    async findNewFiles() {
        ...
        try {
            ...
            if (content)
                content.innerHTML = 'Error searching path.';
        }
    }
    renderFoundFiles() {
        const content = document.getElementById('found-files-content');
        ...
        try {
            ...
            if (info.isIndexing) {
                ...
                if (this.scanResults.length === 0) {
                    ...
                    for (let i = 0; i < info.indexedCount; i++)
                        this.scanResults[i] = { path: 'Indexed', status: 'indexed' };
                }
                this.renderImportControls();
                this.updateProgressBar();
            }
