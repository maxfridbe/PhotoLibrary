...
export class LibraryManager {
    constructor() {
        ...
        hub.sub(ps.PHOTO_IMPORTED, (data) => {
            ...
            if (this.lastImportTime > 0) {
                ...
                if (this.importDurations.length > this.MAX_N)
                    this.importDurations.shift();
    ...
    _renderStats() {
        if (!this.statsVNode)
            return;
        this.statsVNode = patch(this.statsVNode, LibraryStatistics(this.infoCache, this.isBackingUp, async () => {
            if (this.isBackingUp)
                return;
