...
export function LibraryStatistics(info, isBackingUp = false, onBackup) {
...
function renderStats(info, isBackingUp, onBackup) {
    ...
    const backupList = info.backups && info.backups.length > 0 ? [
        ...
        h('div', { style: { fontSize: '0.85em', background: 'var(--bg-panel-alt)', borderRadius: '4px', border: '1px solid var(--border-dim)', overflow: 'hidden' } }, info.backups.map((b, i) => h('div', {
            key: 'backup-' + i,
