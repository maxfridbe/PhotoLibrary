...
function renderScanTable(results, isIndexing) {
    let activeIndex = -1;
    if (isIndexing) {
        ...
        if (activeIndex === -1 && results.length > 0)
            activeIndex = 0;
    }
...
function renderImportControls(isIndexing, isCancelling, results, currentPath, onIndex, onCancel, lastDuration = 0, estimatedTime = 0) {
    if (isIndexing) {
        const indexed = results.filter((r) => r.status === 'indexed').length;
        const total = results.length;
        const percent = total > 0 ? (indexed / total) * 100 : 0;
        const nextItem = results.find((r) => r.status === 'pending');
