    ...
    public static async Task StartAsync(int port, IDatabaseManager dbManager, IPreviewManager previewManager, ICameraManager cameraManager, ILoggerFactory loggerFactory, string bindAddr = "localhost", string configPath = "", string runtimeMode = "WebHost", string mapTilesPath = "")
    {
        ...
        _ = Task.Run(async () => {
            while (true) {
                ...
                _logger?.LogInformation("[MONITOR] Memory: WorkingSet={WS}MB, Private={Private}MB", workingSet, privateBytes);
            }
        });
        ...
        _commLayer = new CommunicationLayer(dbManager, previewManager, cameraManager, loggerFactory, configPath, (msg) => Broadcast(msg), _activeTasks, runtimeMode, mapTilesPath);
        ...
        app.MapGet("/tiles/{z}/{x}/{y}.pbf", async (int z, int x, int y, HttpContext context) => {
            var bytes = await _commLayer?.GetMapTileAsync(z, x, y)!;
                ...
                if (wsResult.MessageType == WebSocketMessageType.Text)
                {
                    ...
                    _ = Task.Run(async () => {
                        try {
                            ...
                            if (req != null && _commLayer != null)
                            {
                                long beforeSend = Stopwatch.GetTimestamp();
                                byte[] payload = await _commLayer.GetImageAsync(req, ct);
